//! [`Command`] for creating a new [`contract::ManagementForRent`].
...
/// [`Command`] for creating a new [`contract::ManagementForRent`].
#[derive(Clone, Debug)]
pub struct CreateManagementForRentContract {
    ...
    /// Expected price for rent a [`Realty`].
    pub expected_price: Money,

    /// Expected deposit to be paid once [`Realty`] is rented.
...
impl<Db> Command<CreateManagementForRentContract> for Service<Db>
where
        ...
        + Database<Commit, Err = Traced<database::Error>>,
{
    ...
    async fn execute(
        &self,
        cmd: CreateManagementForRentContract,
    ) -> Result<Self::Ok, Self::Err> {
        use ExecutionError as E;

        let CreateManagementForRentContract {
            realty_id,
            landlord_id,
            employer_id,
            name,
        ...
        let contract = Contract::from(contract::ManagementForRent {
            id: contract::Id::new(),
            name,
            description,
            realty_id: realty.id,
...
/// Error of [`CreateManagementForRentContract`] [`Command`] execution.
